...
if NR_WAGO_TOP == 0: wago_height_t = 0 
else: wago_height_t = WAGO_HEIGHT + WAGO_FIX_HEIGHT
if NR_WAGO_BOTTOM == 0: wago_height_b = 0 
else: wago_height_b = WAGO_HEIGHT + WAGO_FIX_HEIGHT
screw_positions = [ 
           (WAGO_POS_DEPTH_UPPER + wago_height_t + SCREW_BLOCK_SIZE/2,  DIN_HEIGHT/2-WAGO_LIP_LENGTH - CASE_THICKNESS - SCREW_BLOCK_SIZE/2, ),
           (WAGO_POS_DEPTH_LOWER + wago_height_b + SCREW_BLOCK_SIZE/2, -DIN_HEIGHT/2+WAGO_LIP_LENGTH + CASE_THICKNESS + SCREW_BLOCK_SIZE/2, ),
...
if NR_WAGO_BOTTOM <=0: text_lower = dummy_cutout
...
for i in results: 
    print(f"{i}: \n {results[i]} \n ")
    results[i].rotate(*rotations[i]).translate(*translations[i]).export(Path(os.path.basename(__file__)).stem+"_"+CONFIG_NAME+"_"+i+".stl")
small_parts.export(Path(os.path.basename(__file__)).stem+"_"+CONFIG_NAME+"_small_parts.stl")
...
result.export(Path(os.path.basename(__file__)).stem+"_"+CONFIG_NAME+"_"+"assbly"+".step")

compound = result.toCompound()
compound.export(Path(os.path.basename(__file__)).stem+"_"+CONFIG_NAME+"_"+"compound"+".step")
